...
public class ScrollSlidingTabStrip extends HorizontalScrollView {
	...
	public ScrollSlidingTabStrip(Context context, AttributeSet attrs) {
		this(context, attrs, 0);
	}

	public ScrollSlidingTabStrip(Context context, AttributeSet attrs, int defStyle) {
...
//		//调整和文字一样宽度，效果不理想
//		int currentTabWidth = currentTab.getWidth();
//		int paddingLeft = currentTab.getPaddingLeft();
//		int paddingRight = currentTab.getPaddingRight();
//		float diff = (lineRight - lineLeft - currentTabWidth + paddingLeft + paddingRight) / 2;
//		
//		canvas.drawRect(lineLeft + diff, height - indicatorHeight, lineRight - diff, height, rectPaint);

		
		/**
		 * 在这里更改底部滑动块的长度
		 * 现改为左右都减去宽度的四分之一，显示一半长度
		 */
		float len = (lineRight - lineLeft) / 3;
		canvas.drawRect(lineLeft + len, height - indicatorHeight, lineRight - len, height, rectPaint);
