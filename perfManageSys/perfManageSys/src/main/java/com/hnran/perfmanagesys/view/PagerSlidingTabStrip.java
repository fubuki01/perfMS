...
public class PagerSlidingTabStrip extends HorizontalScrollView {
		...
		for (int i = 0; i < tabCount; i++) {
			...
			if (v instanceof TextView) {
				...
				if (i == selectedPosition) {
					...
					tab.setTextSize(TypedValue.COMPLEX_UNIT_PX, tabTextSize + 1);
				}
			}
		}
	...
	protected void onDraw(Canvas canvas) {
		...
		int l = tabsContainer.getChildCount();
...
//		canvas.drawRect(lineLeft + diff, height - indicatorHeight, lineRight - diff, height, rectPaint);
	...
	private class PageListener implements OnPageChangeListener {
		...
		public void onPageScrolled(int position, float positionOffset, int positionOffsetPixels) {
			...
			float w = child != null ? child.getWidth() : 0;
			
			scrollToChild(position, (int) (positionOffset * w));
