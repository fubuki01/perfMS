package com.readboy.ssm.po;

import java.math.BigDecimal;

public class PerformanceCkYzkhgzMx {
	
	private String tjrq;
	private String zzbz;
	private Long gwbz;
	private String yggh;
	private String zjhm;
	private String jgdm;
    private BigDecimal qmye;
    private BigDecimal clnrp;
    private BigDecimal qmnrp;
    private BigDecimal yzkhdj;
    private BigDecimal yzkhdw;
    private BigDecimal yzkhgz;
    private BigDecimal yzkhljgz;
    private BigDecimal ghbl;
    private String khbh;
    private BigDecimal clye;
    private BigDecimal clyrp;
    private BigDecimal qmyrp;
    private String khmc;
    private String zzjc;				//数据库表无，显示要用
    
	public String getZzjc() {
		return zzjc;
	}
	public void setZzjc(String zzjc) {
		this.zzjc = zzjc;
	}
	public String getTjrq() {
		return tjrq;
	}
	public String getZzbz() {
		return zzbz;
	}
	public Long getGwbz() {
		return gwbz;
	}
	public String getYggh() {
		return yggh;
	}
	public String getZjhm() {
		return zjhm;
	}
	public String getJgdm() {
		return jgdm;
	}
	public BigDecimal getQmye() {
		return qmye;
	}
	public BigDecimal getClnrp() {
		return clnrp;
	}
	public BigDecimal getQmnrp() {
		return qmnrp;
	}
	public BigDecimal getYzkhdj() {
		return yzkhdj;
	}
	public BigDecimal getYzkhdw() {
		return yzkhdw;
	}
	public BigDecimal getYzkhgz() {
		return yzkhgz;
	}
	public BigDecimal getYzkhljgz() {
		return yzkhljgz;
	}
	public BigDecimal getGhbl() {
		return ghbl;
	}
	public String getKhbh() {
		return khbh;
	}
	public BigDecimal getClye() {
		return clye;
	}
	public BigDecimal getClyrp() {
		return clyrp;
	}
	public BigDecimal getQmyrp() {
		return qmyrp;
	}
	public String getKhmc() {
		return khmc;
	}
	public void setTjrq(String tjrq) {
		this.tjrq = tjrq;
	}
	public void setZzbz(String zzbz) {
		this.zzbz = zzbz;
	}
	public void setGwbz(Long gwbz) {
		this.gwbz = gwbz;
	}
	public void setYggh(String yggh) {
		this.yggh = yggh;
	}
	public void setZjhm(String zjhm) {
		this.zjhm = zjhm;
	}
	public void setJgdm(String jgdm) {
		this.jgdm = jgdm;
	}
	public void setQmye(BigDecimal qmye) {
		this.qmye = qmye;
	}
	public void setClnrp(BigDecimal clnrp) {
		this.clnrp = clnrp;
	}
	public void setQmnrp(BigDecimal qmnrp) {
		this.qmnrp = qmnrp;
	}
	public void setYzkhdj(BigDecimal yzkhdj) {
		this.yzkhdj = yzkhdj;
	}
	public void setYzkhdw(BigDecimal yzkhdw) {
		this.yzkhdw = yzkhdw;
	}
	public void setYzkhgz(BigDecimal yzkhgz) {
		this.yzkhgz = yzkhgz;
	}
	public void setYzkhljgz(BigDecimal yzkhljgz) {
		this.yzkhljgz = yzkhljgz;
	}
	public void setGhbl(BigDecimal ghbl) {
		this.ghbl = ghbl;
	}
	public void setKhbh(String khbh) {
		this.khbh = khbh;
	}
	public void setClye(BigDecimal clye) {
		this.clye = clye;
	}
	public void setClyrp(BigDecimal clyrp) {
		this.clyrp = clyrp;
	}
	public void setQmyrp(BigDecimal qmyrp) {
		this.qmyrp = qmyrp;
	}
	public void setKhmc(String khmc) {
		this.khmc = khmc;
	}
	
}